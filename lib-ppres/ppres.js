	...
	**/
	...
	result.done( function( data ){
		...
		jQuery.each(providers, function (i,v){
			ss_feature = mainObj.getFeatureByProvider(  feature_types_list, v  );
			target_div.append('<h1>'+v+'</h1>');
			target_div.append( JSON.stringify(ss_feature));
		});	
	});
	 	...
	 	jQuery.each( feature_list, function(index,  feature_type) {
	 		 if ( jQuery.isArray(feature_type.featureProviderGroup )){
				...
				if (! feature  === undefined)
	 			...
	 			if (feature_type.featureProviderGroup === undefined)
	 				selector = feature_type;
...
function dataSource( file_obj ){
	...
	this.loadData = function() {
		...
		return( jQuery.ajax( { url : this.file_path, 
