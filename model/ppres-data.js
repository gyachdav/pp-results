...
function PPResData() {
    ...
    var setJsonData = function(json) {
        ...
        // NOTE: GY the policy is now to remove PROF results if REPROF results exist


        // console.log(json_data);
        // var to_delete = self.getFeatureByProvider(json_data.entry.featureTypeGroup, "PROFsec");

        // json_data.entry.featureTypeGroup,
    ...
    return {
        ...
        getSecondaryStructure: function(argument) {
            // NOTE 2014_20_03: we are now returning reprof results when avaialable
            var ss_feature = this.getFeatureByProvider(this.getFeatureTypeGroup(), "REPROFSec");
            if (ss_feature.feature === undefined)
                ss_feature = this.getFeatureByProvider(this.getFeatureTypeGroup(), "PROFsec");
            return ss_feature;
        ...
        getSolvAcc: function(argument) {
            var solvAccFeature = this.getFeatureByProvider(this.getFeatureTypeGroup(), "REPROFAcc");
            if (!solvAccFeature.solventAccessibility || solvAccFeature.solventAccessibility === undefined)
                solvAccFeature = this.getFeatureByProvider(this.getFeatureTypeGroup(), "PROFacc");
