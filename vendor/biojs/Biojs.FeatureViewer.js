Biojs.FeatureViewer = Biojs.extend({
    ...
    eventTypes: ["onFeatureClick", "onFeatureOn", "onFeatureOff", "onFeatureSelected", "onFeatureUnselected"],
    ...
    customize: function (selectedStyle, paintHorizontalGrid, paintVerticalGrid) {
        ...
        if ((selectedStyle == "") || ((selectedStyle != "") && (selectedStyle == config.style))) {
            if (paintVerticalGrid != config.verticalGrid) {
                config.verticalGrid = paintVerticalGrid;
                if (paintHorizontalGrid != config.horizontalGrid) {
                    config.horizontalGrid = paintHorizontalGrid;
                    if (paintVerticalGrid && paintHorizontalGrid) {
    ...
    exportFeaturesToImage: function () {
        ...
        if (jQuery.browser.msie) {
            ...
            arguments = arguments + "&dasReference=" + config.dasReference + "&dasSources=" + config.dasSources + "&width=" + config.sizeX + "&option=image" + "&hgrid=" + config.horizontalGrid + "&vgrid=" + config.verticalGrid + "&style=" + config.style;
            ...
            this.$imageExported = jQuery('<div id="uniprotFeaturePainter-imageExportedDiv"></div>').html('<img id="uniprotFeaturePainter-imageExported" alt="exported image" src="' + dataURL + '"/>').dialog({
                autoOpen: true,
    ...
    _init: function () {
        ...
        var holder = document.getElementById("uniprotFeaturePainter-holder");
        if (!holder) {
            this.$errorMsg = jQuery('<div id="uniprotFeaturePainter-errorInit"></div>').html('There was an unexpected failure, the image cannot be displayed.').dialog({
                autoOpen: true,
                title: 'Error',
                modal: true
            });
    ...
    _paintJson: function (withoutZoom) {
        ...
        for (var i = 0; i < this.opt.json.featuresArray.length; i++) {
            ...
            if (!((elem.featureEnd < config.requestedStart) || (elem.featureStart > config.requestedStop))) {
    ...
    _paintRuler: function () {
        ...
        var divisions = 1.0 * config.rulerLength / config.pixelsDivision;
        var divisionValue = 1.0 * (config.requestedStop - config.requestedStart + 1) / divisions;
        var divisionSize = config.rulerLength / (divisions * 10);
        divisions = Math.round(divisions);
        divisionValue = Math.round(divisionValue);
        for (var i = 1; i < divisions * 10; i++) {
    ...
    _featureClick: function (onlySelect, myself, raphaelObj, featureObj) {
        ...
        if (raphaelObj == myself._previousClickedShape) {
            if (myself._originalColor == myself.opt.selectionColor) {
                myself._originalColor = myself._previousClickedColor;
                raphaelObj.attr({
                    stroke: myself._previousClickedColor,
                    fill: myself._previousClickedColor
                });
            ...
            if (myself._previousClickedShape && (myself._previousClickedShape != "")) {
                try {
                    myself._previousClickedShape.attr({
                        stroke: myself._previousClickedColor,
                        fill: myself._previousClickedColor
                    });
                    myself._previousClickedShape.animate({
                        "fill-opacity": .5
                    }, 500);
    ...
    _paint: function (obj, sequenceLineY) {
        ...
        if (sequenceLineY) {
            ...
            if (this.opt.selectFeatureOnMouseClick) {
                if (this.opt.highlightFeatureOnMouseOver) {
                    _clickedShape = false;
        ...
        if (this.opt.showFeatureTooltipOnMouseOver == true) {
            if (obj.featureId) {
                ...
                var tooltip = obj.featureLabel + " (" + obj.featureStart + ", " + obj.featureEnd + "; length " + (obj.featureEnd - obj.featureStart + 1) + ")" + "<br/>Type: " + obj.featureTypeLabel + " - " + obj.typeCode + " - " + obj.typeCategory + "<br/>Evidence: " + obj.evidenceText + " - " + obj.evidenceCode;
    ...
    _paintHorizontalGridLines: function (horizontalGrid, horizontalGridNumLines, gridLineHeight, nonOverlapping, leftMargin, rightMargin, sequenceLineY, rulerY, sizeX, raphael) {
        if ((horizontalGrid == 'true') || (horizontalGrid == true)) {
            for (i = 1; i <= horizontalGridNumLines; i++) {
                if (!nonOverlapping) {
                    var line = raphael.path("M" + leftMargin + " " + (sequenceLineY + i * gridLineHeight) + "L" + (sizeX - rightMargin) + " " + (sequenceLineY + i * gridLineHeight));
                    line.attr({
                        stroke: "black",
                        "stroke-width": 0.7,
                        "stroke-opacity": this._GRID_OPACITY
                    });
                    var lineUp = raphael.path("M" + leftMargin + " " + (sequenceLineY - i * gridLineHeight) + "L" + (sizeX - rightMargin) + " " + (sequenceLineY - i * gridLineHeight));
                    lineUp.attr({
                        stroke: "black",
                        "stroke-width": 0.7,
                        "stroke-opacity": this._GRID_OPACITY
                    ...
                    var lineUp = raphael.path("M" + leftMargin + " " + (rulerY + (i + 1) * gridLineHeight) + "L" + (sizeX - rightMargin) + " " + (rulerY + (i + 1) * gridLineHeight));
                    lineUp.attr({
                        stroke: "black",
                        "stroke-width": 0.7,
                        "stroke-opacity": this._GRID_OPACITY
            ...
            if (!nonOverlapping && (horizontalGridNumLines != 0)) {
                var line = raphael.path("M" + leftMargin + " " + (sequenceLineY + (horizontalGridNumLines + 1) * gridLineHeight) + "L" + (sizeX - rightMargin) + " " + (sequenceLineY + (horizontalGridNumLines + 1) * gridLineHeight));
                line.attr({
                    stroke: "black",
                    "stroke-width": 0.7,
                    "stroke-opacity": this._GRID_OPACITY
            ...
            if (nonOverlapping && (horizontalGridNumLines != 0)) {
                var line = raphael.path("M" + leftMargin + " " + (rulerY + (horizontalGridNumLines + 2) * gridLineHeight) + "L" + (sizeX - rightMargin) + " " + (rulerY + (horizontalGridNumLines + 2) * gridLineHeight));
                line.attr({
                    stroke: "black",
                    "stroke-width": 0.7,
                    "stroke-opacity": this._GRID_OPACITY
    ...
    _paintVerticalGridLines: function (verticalGridLineLength, stop, start, rulerLength, rulerY, pixelsDivision, leftMargin, raphael) {
        var divisions = 1.0 * rulerLength / pixelsDivision;
        var divisionValue = 1.0 * (stop - start + 1) / divisions;
        var divisionSize = rulerLength / (divisions * 10);
        divisions = Math.round(divisions);
        divisionValue = Math.round(divisionValue);
        for (var i = 1; i < divisions * 10; i++) {
    ...
    initRaphael: function () {
        ...
        Raphael.fn.uniprotFeaturePainter_connection = function (obj1, obj2, line, bg) {
            ...
            if ((!bb1.x) && (obj1.type == "circle")) {
                bb1 = {
                    height: obj1.attrs.r * 2,
                    width: obj1.attrs.r * 2,
                    x: obj1.attrs.cx - obj1.attrs.r,
                    y: obj1.attrs.cy - obj1.attrs.r
                };
            }
            var bb2 = obj2.getBBox();
            if ((!bb2.x) && (obj2.type == "circle")) {
                bb2 = {
                    height: obj2.attrs.r * 2,
                    width: obj2.attrs.r * 2,
                    x: obj2.attrs.cx - obj2.attrs.r,
                    y: obj2.attrs.cy - obj2.attrs.r
                };
            }
            var p = [{
                    x: bb1.x + bb1.width / 2,
